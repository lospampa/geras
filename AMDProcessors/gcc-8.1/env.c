...
static bool parse_geras(const char *name, int *pvalue, bool allow_zero){
        char *env = getenv(name);
        if(env == NULL){
                printf("GERAS: Disabled\n");
                geras_init(3,0);
                *pvalue = -1;
                return false;
        }
        if( (strcmp("aging",env) == 0) || (strcmp("AGING",env) == 0)){
                printf("GERAS - OpenMP Application Optimized for AGING\n");
                *pvalue = 0;
        }else{
                printf("GERAS - Optimization not recognized!\n");
                printf(" -- OpenMP application not optimized\n");
                *pvalue = -1;
                printf("\n\t\tPlease:\n");
                printf("\t\tTo optimize aging: export OMP_GERAS=AGING or export OMP_GERAS=aging\n");
        }
        return true;
}
...
initialize_env (void)
{
  ...
  parse_geras("OMP_GERAS", &gomp_global_icv.geras_var,true);
if(gomp_global_icv.geras_var != -1){
        parse_int("OMP_GERAS_START_SEARCH", &gomp_global_icv.geras_start_search, true);
          geras_init(gomp_global_icv.geras_var, gomp_global_icv.geras_start_search);
   }
